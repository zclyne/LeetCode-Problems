...
// Solution 2: BFS
// 把数字1~n看作一张图的所有节点，当且仅当两数之差为一个完全平方数时，两节点间有一条边，初始时，先把所有完全平方数存到数组perfectSquares中，并创建数组visited用于记录某个数是否已经被访问过
// 若visited[n]，表明n本身是一个完全平方数，直接返回1
// 把perfectSquares中的所有数加入队列中，开始循环。每次循环时，取出队列中的所有元素，并将其与各个完全平方数相加得到一个新的数。如果这个数小于n且还未被访问过，就将它加入队列
